...
def load_model():
    ...
    model.eval()
    _state.update(model=model, classes=ckpt["classes"], img_size=ckpt["img_size"],
                   hazardous=set(ckpt.get("hazardous", [])), device=device)
    print(f"Loaded model. Classes={ckpt['classes']}")

class PredictionResponse(BaseModel):
    predicted_class: str
    confidence: float
    top_k: List[dict]
    is_hazardous_prediction: bool
    possible_hazard_flag: bool
    note: str = ""

@app.get("/health")
def health():
    return {"status": "ok", "classes": _state.get("classes"),
            "hazardous_classes": sorted(_state.get("hazardous", []))}
...
async def predict(file: UploadFile = File(...)):
    ...
    is_hazardous_prediction = predicted_class in _state["hazardous"]
    possible_hazard_flag = any(
        classes[i] in _state["hazardous"] and probs[i].item() >= HAZARD_SECONDARY_THRESHOLD
        for i in range(len(classes))
    ) and not is_hazardous_prediction

    note = ""
    if is_hazardous_prediction:
        note = "Predicted a hazardous category - route accordingly, do not auto-clear."
    elif possible_hazard_flag:
        note = "Hazardous class scored above secondary threshold - recommend manual check."

    return PredictionResponse(predicted_class=predicted_class, confidence=float(top_prob.item()),
